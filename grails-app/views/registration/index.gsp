    ...
    <body>
        ...
        <g:form controller="registration" action="registrationstep1">
        	...
        	<label for="firstname">firstname</label>
        		<input type="text" name="partyuser.firstname" value="${fieldValue(bean:partyuser,field:'firstname')}" />
        	</div>
        	
        	<br/>
        	<div class="prop ${hasErrors(bean:partyuser,field:'lastname', 'errors')}">
        	<label for="lastname">lastname</label>
        	<input type="text" name="partyuser.lastname" value="${fieldValue(bean:partyuser,field:'lastname')}" />
        	</div>
        	
        	<br/>
        	<div class="prop ${hasErrors(bean:partyuser,field:'email', 'errors')}">
        		<label for="email">email</label>
        		<input type="text" name="partyuser.email" value="${fieldValue(bean:partyuser,field:'email')}" />
			</div>
			
        	<br/>
        	<div class="prop ${hasErrors(bean:principal,field:'username', 'errors')}">
        		<label for="username">username</label>
        		<input type="text" name="principal.username"  value="${fieldValue(bean:principal,field:'username')}" />
        	</div>
        	<br/>
        	<div class="prop ${hasErrors(bean:principal,field:'password', 'errors')}">
        		<label for="password">password</label>
        		<input type="password" name="principal.password"/>
        	</div>
        	
        	<br/>
        	<label for="confirmpassword">Confirm password</label>
        	<input type="password" name="principal.confirm" />
